...
/**
  ******************************************************************************
  * @file           : main.c
  * @brief          : Main program body — Semaphore Demo (replaces Queue)
  ******************************************************************************
  */
...
/* ------------------------------------------------------------------ */
/*  Binary Semaphore replaces the Message Queue               */
/* ------------------------------------------------------------------ */
...
int main(void)
{
  ...
  /* ------------------------------------------------------------------ */
  /*  CHANGED: Create binary semaphore (max=1, init=0)                  */
  /*  init=0 means ConsumerTask will block immediately until ButtonTask  */
  /*  releases it.                                                       */
  /* ------------------------------------------------------------------ */
...
void ButtonTask2(void *argument)
{
  ...
  for(;;)
  {
    ...
    {
      /* ------------------------------------------------------------------ */
      /*  CHANGED: Release semaphore instead of putting to queue            */
      /*  No data is sent — just a signal: "something happened, go!"        */
      /* ------------------------------------------------------------------ */
      osSemaphoreRelease(ButtonSemaphoreHandle);

      osMutexAcquire(UartMutexHandle, osWaitForever);
      UART_Print("[Button] Pressed! Semaphore released\r\n");
      osMutexRelease(UartMutexHandle);
...
void ConsumerTask3(void *argument)
{
  ...
  {
    /* ------------------------------------------------------------------ */
    /*  CHANGED: Block on semaphore instead of queue                      */
    /*  Task sleeps here using zero CPU until ButtonTask releases it.     */
    /* ------------------------------------------------------------------ */
    osSemaphoreAcquire(ButtonSemaphoreHandle, osWaitForever);

    osMutexAcquire(UartMutexHandle, osWaitForever);
    UART_Print("[Consumer] Semaphore acquired! Button was pressed\r\n");
    osMutexRelease(UartMutexHandle);
  }
}
