...
/**
  ******************************************************************************
  * @file           : main.c
  * @brief          : Main program body — Task Notification Demo (replaces Semaphore)
  ******************************************************************************
  */
...
/* ------------------------------------------------------------------ */
/*  CHANGED: Added native FreeRTOS headers for Task Notifications      */
/* ------------------------------------------------------------------ */
#include "FreeRTOS.h"
#include "task.h"
...
/* ------------------------------------------------------------------ */
/*  CHANGED: Semaphore is completely gone. No object needed.           */
/* ------------------------------------------------------------------ */
...
void LedBlinkTask1(void *argument);
...
int main(void)
{
  ...
  /* ------------------------------------------------------------------ */
  /*  CHANGED: No semaphore creation needed at all                      */
  /* ------------------------------------------------------------------ */

  /* Create Tasks */
  defaultTaskHandle  = osThreadNew(StartDefaultTask, NULL, &defaultTask_attributes);
  LedBlinkTaskHandle = osThreadNew(LedBlinkTask1,    NULL, &LedBlinkTask_attributes);
...
void ButtonTask2(void *argument)
{
  ...
  for(;;)
  {
    ...
    {
      /* ------------------------------------------------------------------ */
      /*  CHANGED: Directly notify ConsumerTask — no semaphore object       */
      /*  xTaskNotifyGive() knocks directly on ConsumerTask's door          */
      /*  ConsumerTaskHandle is the target — only that task gets notified   */
      /* ------------------------------------------------------------------ */
...
void ConsumerTask3(void *argument)
{
  ...
  {
    /* ------------------------------------------------------------------ */
    /*  CHANGED: Wait on own built-in notification counter                */
    /*  pdTRUE = clear the counter after waking (reset back to 0)        */
    /*  portMAX_DELAY = wait forever until notified                       */
    /*  Task sleeps here using zero CPU until ButtonTask knocks           */
    /* ------------------------------------------------------------------ */
